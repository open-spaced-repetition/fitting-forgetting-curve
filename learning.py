...
def fit_forgetting_curve(user_id: int):
    ...
    df = dataset[(dataset["t_history"] == "0,0,1") & (dataset["r_history"] == "3,3,3")]
    if df.empty:
        return None
    ...
    if PLOT:
        ...
        plt.legend()
        Path("learning_plots").mkdir(parents=True, exist_ok=True)
        plt.savefig(f"learning_plots/{user_id}.png")
        plt.close()
...
if __name__ == "__main__":
    ...
    final_results.to_csv("learning_results.csv", index=False)
